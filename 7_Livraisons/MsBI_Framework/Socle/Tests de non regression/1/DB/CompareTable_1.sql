...
GO
*/
CREATE PROCEDURE [Adm].[CompareTable] (
	@Domaine		VARCHAR(50),
...
Begin 
	...
	SET @Query =		 N'INSERT INTO [Adm].Tests_Resultats (Domaine, Colonne, Ecart, DtResultat, isCurrent, ValeurCle1, ValeurCle2, ValeurCle3, ValeurCle4) '
	SET @Query = @Query + 'SELECT '''+@Domaine+''', '''+@KeyGlobal+''' AS Colonne, ''En trop'' as Ecart, GETDATE() AS DtResultat, 1 AS isCurrent, '
	...
	SET @Query =		 N'INSERT INTO [Adm].Tests_Resultats (Domaine, Colonne, Ecart, DtResultat, isCurrent, ValeurCle1, ValeurCle2, ValeurCle3, ValeurCle4) '
	SET @Query = @Query + 'SELECT '''+@Domaine+''', '''+@KeyGlobal+''' AS Colonne, ''Manquant'' as Ecart, GETDATE() AS DtResultat, 1 AS isCurrent, '
	...
	BEGIN
		...
		SET @Query =		 N'INSERT INTO [Adm].Tests_Resultats (Domaine, Colonne, Ecart, DtResultat, isCurrent, ValeurCle1, ValeurCle2, ValeurCle3, ValeurCle4) '
		SET @Query = @Query + 'SELECT '''+@Domaine+''', '''+@Column_name+''' AS Colonne, ''Different'' as Ecart, GETDATE() AS DtResultat, 1 AS isCurrent, '
		SET @Query = @Query + '		  CMP.'+@Key1+' AS ValeurCle1 '
		IF @NbKey > 1
		BEGIN
		SET @Query = @Query + '      ,CMP.'+@Key2+' AS ValeurCle2 '
