...
Begin 
	...
	/* Preparation des index s'ils n'existent pas */
	SET @Query = 'IF NOT EXISTS (SELECT 1 '
	SET @Query = @Query + 'FROM '+@DBCible+'.[sys].[indexes] IND '
	SET @Query = @Query + 'JOIN '+@DBCible+'.[sys].[objects] OBJ '
	SET @Query = @Query + '  ON IND.object_id = OBJ.object_id '
	SET @Query = @Query + 'JOIN '+@DBCible+'.[sys].[schemas] SCH '
	SET @Query = @Query + '  ON OBJ.schema_id = SCH.schema_id '
	SET @Query = @Query + 'WHERE OBJ.name = '''+@TableCible+''' '
	SET @Query = @Query + '  AND IND.name = ''TEC_IndexCles'' '
	SET @Query = @Query + '  AND SCH.name = '''+@SchemaCible+''') '
	SET @Query = @Query + 'BEGIN '
	SET @Query = @Query + 'CREATE CLUSTERED INDEX [TEC_IndexCles] ON '+@DBCible+'.'+@SchemaCible+'.'+@TableCible+' ' 
	SET @Query = @Query + '( '
	SET @Query = @Query + '	'+@Key1+' ASC '
	IF @NbKey > 1
	BEGIN
	SET @Query = @Query + ','+@Key2+' ASC '
	END
	IF @NbKey > 3
	BEGIN
	SET @Query = @Query + ','+@Key3+' ASC '
	END
	IF @NbKey > 4
	BEGIN
	SET @Query = @Query + ','+@Key4+' ASC '
	END
	SET @Query = @Query + ')WITH (STATISTICS_NORECOMPUTE  = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY] '
	SET @Query = @Query + 'END '

	EXECUTE sp_executesql @Query
	
	SET @Query = 'IF NOT EXISTS (SELECT 1 '
	SET @Query = @Query + 'FROM '+@DBComp+'.[sys].[indexes] IND '
	SET @Query = @Query + 'JOIN '+@DBComp+'.[sys].[objects] OBJ '
	SET @Query = @Query + '  ON IND.object_id = OBJ.object_id '
	SET @Query = @Query + 'JOIN '+@DBComp+'.[sys].[schemas] SCH '
	SET @Query = @Query + '  ON OBJ.schema_id = SCH.schema_id '
	SET @Query = @Query + 'WHERE OBJ.name = '''+@TableComp+''' '
	SET @Query = @Query + '  AND IND.name = ''TEC_IndexCles'' '
	SET @Query = @Query + '  AND SCH.name = '''+@SchemaComp+''') '
	SET @Query = @Query + 'BEGIN '
	SET @Query = @Query + 'CREATE CLUSTERED INDEX [TEC_IndexCles] ON '+@DBComp+'.'+@SchemaComp+'.'+@TableComp+' ' 
	SET @Query = @Query + '( '
	SET @Query = @Query + '	'+@Key1+' ASC '
	IF @NbKey > 1
	BEGIN
