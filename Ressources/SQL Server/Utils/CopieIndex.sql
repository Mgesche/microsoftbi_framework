...
SELECT 'CREATE '+ind.Type_Des+' INDEX ['+ind.name+'] ON ['+@SchemaNameDestination+'].['+@TableNameDestination+'] (['+col.name+'] '+CASE WHEN ind_col.is_descending_key = 1 THEN 'DESC' ELSE 'ASC' END +')WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON, FILLFACTOR = '+CAST(ind.fill_factor as VARCHAR)+') ON [PRIMARY]' as requete
